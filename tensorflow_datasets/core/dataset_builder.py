...
class DatasetBuilder(object):
  ...
  **Configuration**: Some `DatasetBuilder`s expose multiple variants of the
  ...
  def _pick_version(self, requested_version):
    ...
    if isinstance(canonical_version, six.string_types):
      canonical_version = utils.Version(canonical_version)
    if not requested_version or canonical_version.match(requested_version):
      return canonical_version
    for version in supported_versions:
      if isinstance(version, six.string_types):
        version = utils.Version(version)
      if version.match(requested_version):
        return version
