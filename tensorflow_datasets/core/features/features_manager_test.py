...
"""Tests for tensorflow_datasets.core.features.features_manager."""
...
class FeaturesManagerTest(testing.TestCase):
  ...
  def test_sequence_rank(self):
    ...
    self.assertEqual(1, features_manager._get_sequence_rank({
    ...
    with self.assertRaisesWithPredicateMatch(
      ...
      features_manager._get_sequence_rank({
          'a': features_lib.TensorInfo(
  ...
  def test_flatten_nested(self):
    ...
    with self.assertRaisesWithPredicateMatch(
        ValueError, 'Expected length 2 do not match input length 3'):
